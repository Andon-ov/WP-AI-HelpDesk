...
4. ТОН: Бъди професионален кулинарен експерт. Отговаряй на български.",
	...
	private function get_site_context( $msg ) {
		...
		$stop_words = array('със', 'във', 'през', 'под', 'над', 'зад', 'между', 'пред', 'върху', 'като', 'кога', 'къде', 'какво', 'защо', 'кой', 'коя', 'кое', 'кои', 'всички', 'всичко', 'няма', 'има', 'естествено', 'може', 'можеш', 'трябва', 'чрез', 'нас', 'вас', 'тях', 'колко', 'струва', 'дали', 'някой', 'нещо', 'някъде', 'никой', 'нито', 'само', 'още', 'вече', 'много', 'малко', 'повече', 'искам', 'търся', 'имате', 'имам', 'дай', 'покажи', 'кажи', 'опиши', 'препоръчай', 'предложи', 'намери', 'намерете', 'виж', 'вижте', 'знаеш', 'знаете', 'искате', 'търсите', 'тук', 'там', 'сега', 'после', 'преди', 'харесвам', 'харесвате', 'интересно', 'интересува', 'относно', 'защото', 'затова', 'понеже', 'обаче', 'също', 'значи', 'иначе', 'въпреки', 'докато', 'когато',
			// Intent verbs (how/what user wants to DO — not content keywords)
			'как', 'искам', 'науча', 'научи', 'научим', 'подобря', 'подобри', 'подобрим', 'намеря', 'разбера', 'разбери', 'помогни', 'помогнете', 'покажете', 'обясни', 'обяснете', 'кажете', 'питам', 'питате', 'питаш', 'хочу', 'моя', 'моят', 'твоя', 'своя', 'нашия', 'нашата', 'техния',
			'абонирам', 'абонирай', 'абонирайте', 'абонираш', 'абонираме', 'регистрирам', 'регистрирай',
		);
	...
	private function generate_item_tags( $id, $type ) {
		...
		switch ( $type ) {
			case 'product':
				// Detect course/lecture vs subscription products by title keywords
				$title_lower = mb_strtolower( get_the_title( $id ) );
				if ( preg_match('/курс|лекция|обучение|маркетинг|меню|академия/u', $title_lower) ) {
					$tags[] = 'продукт цена курс лекция обучение купи';
				} else {
					$tags[] = 'продукт цена абонамент клуб членство купи абонирай';
				}
				break;
			case 'wprm_recipe':
				...
				break;
			case 'glossary':
				$tags[] = 'лексикон термин кулинарен речник дефиниция обяснение';
				break;
			case 'tribe_events':
				$tags[] = 'събитие дата място програма уебинар семинар';
				break;
			case 'sfwd-courses':
			case 'course':
			...
			default:
				...
				$special = array(
					'leksikon'         => 'лексикон речник кулинарен термини дефиниции',
					'за-нас'           => 'екип автори за нас контакти',
					'contact'          => 'контакти връзка',
					'услуги'           => 'услуги консултация',
					'chef-gastro-club' => 'клуб абонамент членство',
					'гастро-академия'  => 'академия курсове обучение',
					'блог'             => 'блог статии публикации',
				);
				if ( isset( $special[ $slug ] ) ) {
					$tags[] = $special[ $slug ];
				} elseif ( mb_strpos( $permalink, '/за-нас/' ) !== false ) {
					// Author/team pages
					$tags[] = 'автор екип преподавател готвач шеф за нас';
				} else {
	...
	private function sync_site_knowledge() {
		...
		foreach ( $post_types as $type ) {
			...
			foreach ( $query->posts as $id ) {
				...
				// Skip technical/system pages that are not useful for the chatbot
				if ( 'page' === $type ) {
					$skip_slugs = array( 'checkout', 'kolichka', 'cart', 'my-account', 'moyat-profil', 'member-login', 'member-logout', 'lost-password', 'register', 'thank-you-page', 'default-redirect-page', 'public-individual-page', 'privacy-policy', 'политика-за-бисквитките-европейски-с', 'финализиране' );
					if ( in_array( $p->post_name, $skip_slugs ) ) continue;
					if ( empty( trim( wp_strip_all_tags( $p->post_content ) ) ) ) continue;
				}
